...
class MambaMixer(nn.Module):
    ...
    def forward(self, hidden_states,contex_len, cache_params: Optional[MambaCache] = None):
        if is_fast_path_available and "cuda" in self.x_proj.weight.device.type:
            return self.cuda_kernels_forward(hidden_states, cache_params)
        return self.slow_forward(hidden_states,contex_len, cache_params)
...
class MambaBlock(nn.Module):
    ...
    def forward(self, hidden_states, contex_len, cache_params: Optional[MambaCache] = None):
        ...
        hidden_states = self.mixer(hidden_states,contex_len, cache_params=cache_params)
...
class MambaModel(MambaPreTrainedModel):
    ...
    def forward(
        ...
        contex_len: Optional[torch.LongTensor] = None,
        ...
        **kwargs,  # `attention_mask` is passed by the tokenizer and we don't want it
        ...
        for mixer_block in self.layers:
            if self.gradient_checkpointing and self.training:
                hidden_states = self._gradient_checkpointing_func(mixer_block.__call__, hidden_states,contex_len, cache_params)
            else:
                hidden_states = mixer_block(hidden_states,contex_len, cache_params=cache_params)
